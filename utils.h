...
template<class unum>
int indexAlphabet(const uchar *in, unum length, uint *charIndex, int& bitsPerChar) {
...
template<class T>
void flipArray(T *a, int n, T val) {
    for (int i = 0; i < n; ++i) {
        a[i] = val - a[i];
    }
}
...
void inplaceShiftedBy1PrefixSum(T *a, S n) {
    T next = 0;
    for (S i = 0; i < n; ++i) {
