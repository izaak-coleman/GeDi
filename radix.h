...
#define MOD_WORDSIZE(p) ((p) & 63)
    ...
    void packInput(const uchar *originalInput, const unum length, const uint *charCode,
        ...
        for (unum i = 0; i < length; ++i) {
            ...
            if (remainBits >= 0) {
                ...
                // but the left shifting will get rid of the extra bits later anyway
...
public:
    Radix(uchar *input, unum n, unum kmerLength = 0) :
            originalInput(input), length(n), bucketPiggyBackBits(unumBits - bitsFor(length)), bucketPiggyBackLimit(
                    ...
                    bucketPiggyBackLimit - 1), kmerLength(kmerLength) {
    }
...
//                "bitsPerFirstPass 18/bitsPerChar*BitsPerChar " <<  ((18 / bitsPerChar) * bitsPerChar) <<
//                "; 16/bitsPerChar*BitsPerChar " <<  ((16 / bitsPerChar) * bitsPerChar) <<
//                endl;
...
private:
    ...
    unum fixEndingZeros(uint *charCode) {
        unum p = 0;
        for (unum i = length - 1; (p < length) && charCode[originalInput[i]] == 0; --i)
            bucketFlag[++p] = 1;
        return p;
    }
